...
This document records the required contract and acceptance matrix. Version 0.1.1 implements installed-route/command inventory, explicit catalogue synchronization, reviewed command planning/execution, durable jobs and artifacts. JCB bindings are materialized from the installed registries through the administrator Operations screen or `joomla:mcp:jcb-sync`; ordinary discovery reads persisted rows. [jcb-surface.json](jcb-surface.json) records source-backed requirements and measured installed evidence. Its `runtimeEnabled: false` means the documentation is never loaded as executable seed data; it does not mean the JCB runtime is disabled. Current installed execution evidence and remaining checks are also recorded in [IMPLEMENTATION.md](../IMPLEMENTATION.md). Unsupported native contracts or absent handlers require explicit diagnostics; they are not successful executable coverage.
...
The installed golden-image inventory at component commit `4109cd371beb66183157d344da651a143a10037c` confirmed **zero registered JCB API routes** for this pinned distribution. The server correctly materialized no entity API bindings. `tests/integration/jcb-api.php` uses the real Joomla `ApiRouter` with fixture registrations to verify mapping, filtering, fixed defaults and unsupported-contract diagnostics; it does not execute a separately installed JCB API controller.

A separately generated or installed JCB API may exist; this observed distribution boundary is **not a claim that JCB has no API**. Such a distribution requires its own pinned manifest, enabled webservices plugin, route defaults, API controllers/serializers, administrator models/forms and live CRUD acceptance. Record that exact distribution/plugin source and version instead of guessing `/v1/componentbuilder/...` paths. Authenticated HTTP access to the reviewed JCB job adapter is a separate surface from these native entity API routes.

### Entity map is not a command registry

The 45-entity factory map routes package definitions to 40 Package areas and five specialized factories. It is neither the complete API resource list nor proof that all five package verbs are registered for every entity. The pinned golden installation registered **111 executable commands** with no unsupported registrations. Acceptance compared every installed command with its persisted native arguments, options and implementation identity on both authority tracks. This mapping count does not mean 111 separate execution scenarios passed. Additional real commands outside the documented families remain in scope; changes are discovered from the actual registry, never from an assumed 225-command cross-product.
...
The following evidence belongs to component commit `75d9685268332241de846935aad2edc2e92c8459` on 24 September 2026. Both [push](https://github.com/joomengine/mcp_component/actions/runs/35983422644) and [PR](https://github.com/joomengine/mcp_component/actions/runs/35983427068) golden-image runs passed. The [completion checklist](https://github.com/joomengine/mcp_component/pull/1#issuecomment-5732685349) records the final documentation/dependency-pin revision and its rerun.

| Scenario | Observed result | Boundary |
| --- | --- | --- |
| Joomla 6.1.3, PHP 8.3/8.4, MySQL 8.4/PostgreSQL 16 | All four [installed core matrix jobs](https://github.com/joomengine/mcp_component/actions/runs/35983426742) passed. | Core HTTP, ACL, administrator, stdio, worker and package lifecycle, plus standalone plugin and HTTPS-client acceptance on each platform. |
| JoomEngine image `octoleo/joomengine:6.1.6-php8.4-apache` | Complete installed JCB, console, remote client and lifecycle acceptance passed. Observed runtime: Joomla 6.1.3, PHP 8.4.25, MariaDB 11.4.13. | The image tag differs from the observed Joomla version. This is the JCB-present platform; it does not certify JCB on PostgreSQL or PHP 8.3. |
| Native CLI compiler and jobs | 299 checks passed, including Demo J6 compile/install, three verified extension archives, owned downloads, stdio disconnection, cross-owner denial, real compiler failure and reconciliation. | One real compiler target/scenario plus native option-contract tests, not every target/option combination. |
| Native package roundtrip | 30 CLI and 30 HTTPS-client checks passed across get/init/pull/push/reset, a linked dependency, overwrite, remote failure, replay and reconciliation. | Native definition roundtrip against a disposable repository fixture. The filesystem-reset limitation above still applies. |
| Generic client over trusted HTTPS | 288 checks passed, including authorized `jcb.execute` grants, native package/compiler jobs, three downloaded ZIPs with validated SHA-256 hashes, ownership denials, failure read-back and reconciliation. | This generic API-authorized track compiled and downloaded archives; local CLI separately tested installation. |
| Worker and installed lifecycle | 17 actual database/process worker checks; 18 plugin checks; seven upgrade checks, 5,430 exact upgraded-seed checks and 23 uninstall checks passed. | Claim races, cancellation and lost-worker cases preserve uncertain effects for inspection; cancellation is not rollback. |

The inspected [evidence artifact](https://github.com/joomengine/mcp_component/actions/runs/35983422644/artifacts/10801311856) records 111 executable command registrations, zero native JCB API routes, plugin `3526cae818803a02971374c044a2e2184f1c2c61`, client `72d02491fe80dadc581d3d9d67b1dfbc917d095d`, and installed compiler source SHA-256 `1c7d2770e13668c8b653c501fcb2962984be84f5d02cf9a626127524ff3697fc`. Permission-schema, native validation-diagnostic, plan-preview, registration-plugin visibility and frame-boundary corrections are included. The current workflow pins include the independently tested Docker Compose correction and finalized companion documentation. Absent API registration, unsupported specialized tasks and the inherited filesystem limitation remain explicit boundaries, not fabricated execution passes.
