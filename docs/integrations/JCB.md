...
### Measured installed evidence and final rerun

The following evidence belongs to component commit `4109cd371beb66183157d344da651a143a10037c` on 22 September 2026. It is an observed baseline; final changes require another complete run.

| Scenario | Observed result | Boundary |
| --- | --- | --- |
| Joomla 6.1.3, PHP 8.3/8.4, MySQL 8.4/PostgreSQL 16 | All four [installed core matrix jobs](https://github.com/joomengine/mcp_component/actions/runs/35756932400) passed. | Core HTTP, ACL, administrator, stdio, worker and package lifecycle evidence. This matrix did not invoke the optional standalone client or plugin acceptance suites. |
| JoomEngine image `octoleo/joomengine:6.1.6-php8.4-apache`, MariaDB 11.4 | Installed JCB and console plugin, native synchronization and core suites passed before the remote scenario failed. | This is the JCB-present platform; it does not certify JCB on PostgreSQL or PHP 8.3. |
...
| Native package roundtrip | 30 checks passed across get/init/pull/push/reset, a linked dependency, overwrite, remote failure, replay and reconciliation. | Native definition roundtrip against a disposable repository fixture. The filesystem-reset limitation above still applies. |
| Generic client over trusted HTTPS | The bridge initialized and discovered the required tools, then the server schema rejected permission scope `jcb.execute`. | Remote package/compiler execution and subsequent golden-image upgrade/uninstall were not completed. |

The [golden-image run](https://github.com/joomengine/mcp_component/actions/runs/35756932424) therefore correctly remained failed. Its [evidence artifact](https://github.com/joomengine/mcp_component/actions/runs/35756932424/artifacts/10708972188) retains the successful local scenarios and the exact remote-schema diagnostic. Complete the permission-contract correction and rerun with the final reviewed component, plugin and client revisions before claiming full installed interoperability. Keep absent native API registration, unsupported tasks and upstream filesystem limitations visible in the final evidence.
